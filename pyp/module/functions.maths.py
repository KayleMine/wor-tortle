...
def from_axis_angle(axis, angle):
  axis = axis / np.linalg.norm(axis) # нормализаци€ оси
...
def rotate_vector_to_target(point_a, point_b, velocity_a):
    # ¬ычисл€ем вектор от точки A к точке B
    vector_ab = point_b - point_a

    # ¬ычисл€ем углы между вектором скорости и вектором AB
    angles_velocity = get_angle_between_vectors(velocity_a, [1, 0, 0])
    angles_ab = get_angle_between_vectors(vector_ab, [1, 0, 0])

    # ¬ычисл€ем разницу между углами
    angles_diff = angles_ab - angles_velocity

    # ѕоворачиваем вектор скорости на разницу углов
